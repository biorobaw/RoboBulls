 ...
 * Modified for Robobulls by Muhaimen Shamsi
 \****************************************/
/* CHANGELOG:
 * 8/11/15 : Changed ID from 8 to 3 and removed old Encoder code
 */
...
// Declare motor pins
#define enablePinLF 22
#define enablePinLB 13
#define enablePinRF 38
#define enablePinRB 50

#define speedPinLF 2
#define speedPinLB 3
#define speedPinRF 44
#define speedPinRB 45
...
#define brakePinLF 24
#define brakePinLB 11
#define brakePinRF 40
#define brakePinRB 52
...
void setSpeeds()
{
  ...
  if(targetLFvel > 0 )
  {
    analogWrite(speedPinLF, abs(targetLFvel));
  ...
  if(targetLBvel > 0 )
  {
    analogWrite(speedPinLB, abs(targetLBvel));
  ...
  if(targetRFvel > 0 )
  {
    analogWrite(speedPinRF, abs(targetRFvel));
  ...
  if(targetRBvel > 0 )
  {
    analogWrite(speedPinRB, abs(targetRBvel));
  ...
  if(Serial.available()>=10)
  {
    ...
    case 'b':                         // Read Packet
      if (Serial.available()>=8)
      {
        ...
        targetLFvelSerial = ((int)Serial.read()-100)*2.5;
        targetLBvelSerial = ((int)Serial.read()-100)*2.5;
        targetRFvelSerial = ((int)Serial.read()-100)*2.5;
        targetRBvelSerial = ((int)Serial.read()-100)*2.5;
