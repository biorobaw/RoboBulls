...
bool GoalieRL::perform()
{//if(team->getID() == 0 ) qInfo() << "GoalieRL Team Blue!!!!";
...
bool GoalieRL::isBallMovingTowardsGoal(std::pair<Point,Point>& lineSegOut)
{
...
bool GoalieRL::botOnBallIsAimedAtOurGoal(Robot* robot, std::pair<Point,Point>& lineSegOut)
{
...
bool GoalieRL::isBallReachable()
{
    return (abs(Measurements::angleBetween(idlePoint, *ball)) <= M_PI_2);
}

bool GoalieRL::shouldClearBall()
{
...
bool GoalieRL::shouldRetrieveBall(Robot* robot)
{
