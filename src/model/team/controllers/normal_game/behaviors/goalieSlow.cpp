...
GoalieSlow::~GoalieSlow()
{
    delete kick_skill;
    delete dribble_skill;
}

bool GoalieSlow::perform()
...
bool GoalieSlow::isBallMovingTowardsGoal(std::pair<Point,Point>& lineSegOut)
{
...
bool GoalieSlow::botOnBallIsAimedAtOurGoal(Robot* robot, std::pair<Point,Point>& lineSegOut)
{
...
bool GoalieSlow::isBallReachable()
{
    return (abs(Measurements::angleBetween(idlePoint, *ball)) <= M_PI_2);
}

bool GoalieSlow::shouldClearBall()
{
...
bool GoalieSlow::shouldRetrieveBall(Robot* robot)
{
