...
void BallPlacementStrategy::assignBehaviors()
{
    if ((game_state->getRefereeCommand() == 17 && team->getID() == ROBOT_TEAM_BLUE) || (game_state->getRefereeCommand() == 16 && team->getID() == ROBOT_TEAM_YELLOW))
    {
        ...
        if(attack1)
        {
            attack1->setBehavior<KickToPointOmni>(target_location);

            std::cout << "attack1 is the kicker" << std::endl;
        }
        else if(attack2)
        {
            attack2->setBehavior<KickToPointOmni>(target_location);

             std::cout << "attack2 is the kicker" << std::endl;
        }
        else if(attack3)
        {
            attack3->setBehavior<KickToPointOmni>(target_location);

             std::cout << "attack3 is the kicker" << std::endl;
        }
        else if(attack4)
        {
            attack4->setBehavior<KickToPointOmni>(target_location);

             std::cout << "attack4 is the kicker" << std::endl;
        }
        else if(wall1)
        {
            wall1->setBehavior<KickToPointOmni>(target_location);

             std::cout << "wall1 is the kicker" << std::endl;
        }
        else if(wall2)
        {
            wall2->setBehavior<KickToPointOmni>(target_location);

             std::cout << "wall2 is the kicker" << std::endl;
        }
        else if(defender1)
        {
            defender1->setBehavior<KickToPointOmni>(target_location);
