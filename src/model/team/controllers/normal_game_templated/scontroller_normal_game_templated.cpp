#include "scontroller_normal_game_templated.h"
#include "strategies/normalgamestrategytemplated.h"
...
SControllerNormalGameTemplated<ATK,SPT>::SControllerNormalGameTemplated(RobotTeam* team, YAML::Node* c_node)
...
template<class ATK, class SPT>
int SControllerNormalGameTemplated<ATK,SPT>::getControllerState(Referee_Command game_command) {
    return cmd_to_state.value(game_command, STOP); // convert command to state, if not defined default to STOP
}

template<class ATK, class SPT>
int SControllerNormalGameTemplated<ATK,SPT>::getNextControllerState(int current_state,int strategy_status){
...
/*
 * ////TODO//////
 * Currently only Normal Game strategy is templated
 * to allow individual behaviors to be changed without
 * copy/pasting code. Add templenting to others.
 * */
template<class ATK, class SPT>
TeamStrategy* SControllerNormalGameTemplated<ATK,SPT>::loadStateStrategy(int state){
    switch (state) {
    ...
    case NORMAL_GAME :   return new NormalGameStrategyTemplated<ATK,SPT>(team);
    }
