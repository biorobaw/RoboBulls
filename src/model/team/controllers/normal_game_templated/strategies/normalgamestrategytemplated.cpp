#include "normalgamestrategytemplated.h"
...
#include "model/team/controllers/normal_game_RL/behaviors/goalieRL.h"
...
#include "model/team/controllers/normal_game_NN/behaviors/attackmainNN.h"
#include "model/team/controllers/normal_game_NN/behaviors/attacksupportNN.h"


//Do not call this... fixes(or masks...) linking errors with the template.
//here: https://www.codeproject.com/Articles/48575/How-to-Define-a-Template-Class-in-a-h-File-and-Imp
//Ive been fighting with this all day
//If needed, define possible combinations of Behaviors/Strategies here.- Justin Rodney
void avoidLinkerIssues()
{
NormalGameStrategyTemplated<AttackMainRL, AttackSupportRL> temp1(nullptr);
NormalGameStrategyTemplated<AttackMainNN, AttackSupportNN> temp2(nullptr);
NormalGameStrategyTemplated<AttackMain, AttackSupport> temp3(nullptr);

}


template<class ATK, class SPT>
NormalGameStrategyTemplated<ATK, SPT>::NormalGameStrategyTemplated(RobotTeam* _team)
    : TeamStrategy(_team)
    , initialBallPos(*ball)
    , our_def_area(false)
    , opp_def_area(true)
...
template<class ATK, class SPT>
void NormalGameStrategyTemplated<ATK, SPT>::assignBehaviors()
{
    ...
    if(attack1)
        attack1->setBehavior<ATK>();
    if(attack2)
        attack2->setBehavior<SPT>();
    if(attack3)
        attack3->setBehavior<SPT>();
    if(attack4)
        attack4->setBehavior<SPT>();
    if(defender1)
...
template<class ATK, class SPT>
void NormalGameStrategyTemplated<ATK, SPT>::runControlCycle()
{
    ...
    case shoot_penalty:
    {
        ...
        if(attack1)
            attack1->setBehavior<AttackSupportRL>();
        if(shooter)
        {
            shooter->setBehavior<ATK>();
            if(dynamic_cast<ATK*>(shooter->getBehavior())->hasKickedToGoal())
            {
    ...
    case attack:
    {
        ...
        if(attack1 != nullptr && attack2 != nullptr)
        {
            ...
            {                                         //other, assign that to attack main
                ...
                attack1->setBehavior<ATK>(); // attackmain is the problem

                main = attack1;
                 //std::cout << "end"<< std::endl;
                attack2->setBehavior<AttackSupportRL>();
                supp = attack2;
...
template<class ATK, class SPT>
void NormalGameStrategyTemplated<ATK, SPT>::assignGoalieIfOk(RobotTeam* team)
{
    Robot* goalie = team->getRobotByRole(RobotRole::GOALIE);
    if(goalie)
        goalie->setBehavior<GoalieRL>();
}
//! @endcond

template<class ATK, class SPT>
QString NormalGameStrategyTemplated<ATK, SPT>::getName(){
    return "normal game";
}

template<class ATK, class SPT>
int NormalGameStrategyTemplated<ATK, SPT>::getStatus(){
    return state;
}
